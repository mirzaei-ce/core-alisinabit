Alisinabit Core version 0.11.0 is now available from:

  <https://alisinabit.org/bin/alisinabit-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Alisinabit Core or other software:
...
defaults to 0.00001. This will cause transactions with fewer ABT/kB fee to be
...
For example, add the following to `alisinabit.conf`:
...
related to the blockchain in the alisinabit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Alisinabit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Alisinabit Core will 
...
For an example script see [alisinabit-submittx](https://github.com/laanwj/alisinabit-submittx).
...
- #5334 `cf87536` libalisinabitconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new alisinabit-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only AlisinabitAmountField
- #5753 `0f44672` Add alisinabit logo to about screen
...
- #5809 `46bfbe7` Add alisinabit-cli man page
...
- #5849 `41259ca` contrib/init/alisinabitd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Alisinabit-Qt.app to Alisinabit-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for alisinabit-tx)
