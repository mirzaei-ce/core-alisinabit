...
###   This script attempts to download the signature file SHA256SUMS.asc from alisinabit.org
...
WORKINGDIR="/tmp/alisinabit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://alisinabit.org/bin/"
VERSIONPREFIX="alisinabit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'alisinabit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Alisinabit Core binary release signing key installed?"
   fi
